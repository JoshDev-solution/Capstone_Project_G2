...
export class DashboardService {
  async getAdminStats() {
    ...
    // 3. Top Services
    const serviceItems = await prisma.billItem.findMany({
      where: {
        serviceId: { not: null },
        bill: { status: 'Paid' }
      },
    ...
    const lowStockItems = products
      .filter((p: any) => p.inventory && p.inventory.quantity <= p.inventory.reorderLevel)
      .sort((a: any, b: any) => (a.inventory?.quantity || 0) - (b.inventory?.quantity || 0))
      .slice(0, 5)
      .map((p: any) => {
    ...
    const recentPayments = await prisma.payment.findMany({ take: 3, orderBy: { createdAt: 'desc' }, include: { bill: { include: { client: { include: { user: true } } } } } });
    const recentUsers = await prisma.user.findMany({ take: 3, orderBy: { createdAt: 'desc' } });
    const recentRefunds = await prisma.refund.findMany({ take: 3, orderBy: { createdAt: 'desc' }, include: { bill: { include: { client: { include: { user: true } } } } } });
