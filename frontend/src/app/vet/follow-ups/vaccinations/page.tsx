...
import { Syringe, Calendar, Search, ArrowRight, User } from "lucide-react";
...
export default function VaccinationsPage() {
  ...
  const fetchRecords = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Filter for vaccinations (case-insensitive)
        const vaxRecords = data.filter((r: any) => 
          r.recordType?.toLowerCase().includes("vaccin") || 
          r.title?.toLowerCase().includes("vaccin")
        );
        setRecords(vaxRecords);
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const calculateNextDue = (dateStr: string) => {
    ...
    d.setFullYear(d.getFullYear() + 1); // Default 1 year for vax
    return d;
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Syringe className="w-6 h-6 text-primary-500" /> Vaccination Tracking
          </h1>
          <p className="text-sm text-neutral-500 mt-1">Monitor administered vaccines and upcoming due dates.</p>
        </div>
        ...
        <table className="w-full text-left text-sm whitespace-nowrap">
          <thead className="bg-neutral-50 dark:bg-neutral-900 border-b border-[var(--card-border)]">
            <tr>
              ...
              <th className="px-6 py-4 font-semibold">Vaccine Administered</th>
              ...
              <tr>
                <td colSpan={5} className="p-10 text-center text-neutral-500">
                  <div className="flex flex-col items-center">
                    <Syringe className="w-10 h-10 mb-3 text-neutral-300" />
                    <p className="font-semibold text-lg text-neutral-700 dark:text-neutral-300">No vaccination records found</p>
